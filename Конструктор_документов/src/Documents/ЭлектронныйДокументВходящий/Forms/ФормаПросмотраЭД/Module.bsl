...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОповеститьОСозданииУведомления" И Параметр = Объект.Ссылка Тогда
		ПоместитьТекстУточненияВОбъект(ЭлектронныйДокумент, ТекстУточнения);
		СтатусОтклонен = Истина;
		ИзменитьСтатусОтклонить();
		Оповестить("ОбновитьСостояниеЭД");
	ИначеЕсли ИмяСобытия = "ЭлектронныйДокументВходящий_ПодборДокументаУчета" И Параметр = ЭлектронныйДокумент Тогда
		ВыполнитьОбработкуОповещенияНаСервере();
		ОбновитьОтображениеДанных();
		Оповестить("ОбновитьСостояниеЭД");
...
Процедура ТекстДокументИБНажатие()
	...
	Если КоличествоДокументов = 0 Тогда
		...
		Иначе
			ВвестиДокументУчетаНаСервере(ЭлектронныйДокумент,"");
		КонецЕсли;
...
Процедура ТекстДокументИБНажатиеЗавершить(Результат, ЭД) Экспорт 
	...
	Если Результат <> Неопределено Тогда
		ВвестиДокументУчетаНаСервере(ЭД, Результат.Значение);	
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ВвестиДокументУчетаНаСервере(ЭлектронныйДокумент, СпособОбработки)
	
	СтруктураПараметров = Новый Структура;
	ЭД = ОбменСКонтрагентамиСлужебный.ПрисоединенныйФайл(ЭлектронныйДокумент);
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	ЕстьВозможностьПодписания = ЕстьПравоОбработки
		И ТребуетсяПодпись(ЭлектронныйДокумент)
