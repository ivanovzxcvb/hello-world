...
Процедура ОткрытьПомощникСозданияОсновныхПроцессов(ТипыОпераций)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru = 'Не выбрана проектная задача.'");
	...
	Если ОкончаниеФакт > Дата(1, 1, 1) Тогда
		...
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Проектная задача завершена. При отправке %1 задача вновь перейдет в стадию выполнения. Отправить %1?'"),
			?(ТипыОпераций = "Исполнение", НСтр("ru = 'на исполнение'"), НСтр("ru = 'в обработку'")));
			
		Режим = Новый СписокЗначений;
		Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Отправить'"));
