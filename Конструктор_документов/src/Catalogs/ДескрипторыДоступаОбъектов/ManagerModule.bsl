...
Функция УдалитьПорциюУстаревшихДанных() Экспорт
	...
	// Запрос, выбирающий дескрипторы, для которых нужно изменить пометку удаления.
	ШаблонЗапросаДляИзмененияПометки =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 500
		|	ДескрипторыДоступаОбъектов.Ссылка КАК Дескриптор,
		|	ДескрипторыДоступаОбъектов.ПометкаУдаления КАК ПометкаДескриптора,
		...
		|		ПО (ТаблицаОбъекта.Ссылка = ДескрипторыДляОбъектов.Объект)
		|ГДЕ
		|	НЕ ДескрипторыДоступаОбъектов.ПометкаУдаления
		|
		|СГРУППИРОВАТЬ ПО
		|	ДескрипторыДоступаОбъектов.Ссылка,
		|	ДескрипторыДоступаОбъектов.ПометкаУдаления
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(ТаблицаОбъекта.ПометкаУдаления) = ИСТИНА
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 500
		...
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(ТаблицаОбъекта.ПометкаУдаления) = ЛОЖЬ";
	
	ВсеЗапросы = Новый Массив;
	ВсеЗапросы.Добавить(ЗапросДляУдаления);
	
	ТипыОбъектов = Метаданные.РегистрыСведений.ДескрипторыДляОбъектов.Измерения.Объект.Тип.Типы();
	Для Каждого Тип Из ТипыОбъектов Цикл
		...
		Если МетаданныеОбъекта <>  Неопределено Тогда
			...
			ТекстЗапроса = СтрЗаменить(ШаблонЗапросаДляИзмененияПометки, "%ИмяТаблицыОбъекта%", ИмяТаблицыОбъекта);
			ЗапросДляИзмененияПометки = Новый Запрос(ТекстЗапроса);
			ВсеЗапросы.Добавить(ЗапросДляИзмененияПометки);
			
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Запрос Из ВсеЗапросы Цикл
		...
		Если ОбработаноЗаписей >= Лимит Тогда
			Прервать;
		КонецЕсли;
...
Процедура ОбработатьВыборкуУстаревшихДанных(Выборка, ЗапросДляПроверкиСсылок, ОбработаноУспешно, ОбработаноСОшибками)
	...
	Пока Выборка.Следующий() Цикл
		...
		НачатьТранзакцию();
		Попытка
			Если Выборка.ЕстьСсылки Тогда
				// Установка пометки удаления, если все объекты, использующие этот дескриптор, помечены
				// или отмена пометки, если есть непомеченные объекты.
				Объект.УстановитьПометкуУдаления(Не Выборка.ПометкаДескриптора);			
