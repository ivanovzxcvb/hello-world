	...
	Если ИмяСобытия = "ЗаписаноПравилоАвтозаполнения" И Параметр.ВидДокумента = Объект.Ссылка
		...
		Для Каждого Строка Из Файлы Цикл
			Если Строка.Ссылка = Параметр.Шаблон Тогда 
				Строка.ПраилоАвтозаполнения = Параметр.Правило;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не МногоШаблонов 
		...
		Для Каждого Строка Из Файлы Цикл
			...
			//кпд (
			НоваяСтрока.кпдКонструкторДокумента = Строка.кпдКонструкторДокумента;
			//кпд )
...
Процедура ФайлыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПравилоАвтозаполнения 
		...
		#КонецЕсли
	//кпд (
	ИначеЕсли Поле = Элементы.кпдКонструкторДокумента Тогда
		 СтандартнаяОбработка = Истина;
	//кпд )
	Иначе
...
Процедура ФайлыПередНачаломИзменения(Элемент, Отказ)
	...
	//кпд (
	Поле = Элементы.Файлы.ТекущийЭлемент;
	Если Поле = Элементы.кпдКонструкторДокумента Тогда
		Отказ = Ложь;
		Возврат;
	КонецЕсли;	
	//кпд )
...
Процедура ВыполнитьИзменениеОрганизации(ВидДокумента, Организация)
	...
	Исключение
		...
		БизнесПроцесс.Наименование = СтрШаблон(
			НСтр("ru ='Не удалось изменить организацию в документах вида ""%1""'"),
			ВидДокумента);
...
Процедура ЗаполнитьСписокФайлов()
	...
	//кпд (
	ТаблицаФайлы.Колонки.Добавить("кпдКонструкторДокумента");
	//кпд )
	...
	Для Каждого Строка Из ШаблонДокумента.ПравилаАвтозаполнения Цикл
		...
		Если НайденнаяСтрока <> Неопределено Тогда 
			...
			//кпд (
			НайденнаяСтрока.кпдКонструкторДокумента = Строка.кпдКонструкторДокумента;
			//кпд )
		КонецЕсли;
	КонецЦикла;
...
//кпд (
&НаКлиенте
Процедура ФайлыкпдКонструкторДокументаПриИзменении(Элемент)
