...
Процедура ОбработатьЭлементыСхемыПроцесса(
	...
	Пока ОчередьЭлементовСхемы.Количество() > 0 Цикл
		...
		// Ищем индекс действия или вложенного процесса среди элементов,
		// т.к. их нужно обрабатывать в первую очередь.
		// Это нужно, чтобы при повторном прохождении одни и тех же путей
		// условия, слияния и разделения использовали актуальные данные о
		// пройденных действиях.
		Для ИндексЭлемента = 0 По ОчередьЭлементовСхемы.Количество() - 1 Цикл
			
			Если ОчередьЭлементовСхемы[ИндексЭлемента].Тип <> 
			...
			ИндексЭлементаДляОбработки = ИндексЭлемента;
			Прервать;
		КонецЦикла;
...
Процедура ОбработатьЭлементСлияниеСхемыПроцесса(ИмяЭлемента, ОчередьЭлементовСхемы, Процесс)
	...
	ПройденныеЭлементы = Новый Соответствие;
	ПройденныеЭлементы.Вставить(ИмяЭлемента, ИмяЭлемента);
	
	ПолучитьИменаПредшествующихДействий(ИмяЭлемента, ИменаДействий, ПройденныеЭлементы, Процесс);
	
	// Проверим их завершенность. Слияние выполняется только при выполнении всех предшествующих действий.
	ВсеДействияПройдены = Истина;
	
	Для Каждого ИмяДействия Из ИменаДействий Цикл
		
		СтрокаЭлемента = СтрокиТаблицыЭлементыСхемы.Получить(ИмяДействия.Ключ);
...
// Получаем имена предшествующих действий.
// Используется для получения действий для элементов слияния.
// Рекурсивная процедура.
//
// Параметры:
//  ИмяЭлемента - Строка - имя текущего элемента, предшественниками которого могут быть действия.
//  ИменаДействий - Соответствие - имена найденных действий (ключ и значение равны имени действия).
//                  В этот параметр помещается результат работы процедуры.
//  Процесс - БизнесПроцессОбъект.КомплексныйПроцесс - объект комплексного процесса, используется
//            для вычисления условий, при обходе схемы процесса в поисках действий.
//
Процедура ПолучитьИменаПредшествующихДействий(Знач ИмяЭлемента, ИменаДействий, ПройденныеЭлементы, Процесс)
	
	Предшественники = ПредшественникиЭлементаСхемы(ИмяЭлемента);
	
	Для Каждого Предшественник Из Предшественники Цикл
		
		ИмяЭлемента = Предшественник.ИмяПредшественника;
		
		Если ПройденныеЭлементы.Получить(ИмяЭлемента) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПройденныеЭлементы.Вставить(ИмяЭлемента, ИмяЭлемента);
		
		ТипЭлемента = СтрокиТаблицыЭлементыСхемы.Получить(ИмяЭлемента).Тип;
		
		Если ТипЭлемента = Перечисления.ТипыЭлементовСхемыКомплексногоПроцесса.Действие
			Или ТипЭлемента = Перечисления.ТипыЭлементовСхемыКомплексногоПроцесса.ВложенныйПроцесс Тогда
			
			ИменаДействий.Вставить(ИмяЭлемента);
			
		ИначеЕсли ТипЭлемента = Перечисления.ТипыЭлементовСхемыКомплексногоПроцесса.Условие Тогда
			
			// Если предшественником является условие, то вычислим условие и при совпадении ожидаемого результата
			// с результатом условия пойдем дальше по текущей ветке пути.
			// Иначе прекращаем обрабатывать текущую ветку.
			
			// Считаем, что если по текущей соединительной линии условия путь невозможен, то
			// из слияния должен быть путь через другую (с другим результатом) соединительную линию с текущим условием.
			
			РезультатУсловияПредшественника = Предшественник.РезультатУсловияПредшественника;
			
			РезультатУсловия = РезультатУсловия(ИмяЭлемента, Процесс);
			
			Если РезультатУсловияПредшественника = РезультатУсловия Тогда
				ПолучитьИменаПредшествующихДействий(ИмяЭлемента, ИменаДействий, ПройденныеЭлементы, Процесс);
			КонецЕсли;
		
		ИначеЕсли ТипЭлемента = Перечисления.ТипыЭлементовСхемыКомплексногоПроцесса.Разделение
			Или ТипЭлемента = Перечисления.ТипыЭлементовСхемыКомплексногоПроцесса.Слияние Тогда
