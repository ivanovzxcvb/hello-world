...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ЗаполнитьПоШаблонуНаСервере("Автоматически");
	...
	Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Видимость = Ложь;
	Элементы.СтраницаИсходящиеЭлектронныеДокументы.Видимость = Ложь;
	Элементы.СтраницаВходящиеЭлектронныеДокументы.Видимость = Ложь;
...
&НаКлиенте
Процедура ВключитьДополнительныеНастройкиСоглашения(Команда)
	
	Элементы.ФормаВключитьРасширенныйРежимНастройкиСоглашения.Видимость = Истина;
	Элементы.СтраницаИсходящиеЭлектронныеДокументы.Видимость = Истина;
	Элементы.СтраницаВходящиеЭлектронныеДокументы.Видимость = Истина;
	Элементы.ФормаВключитьДополнительныеНастройкиСоглашения.Видимость = Ложь;
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		Если Не Объект.ИспользоватьУПД Тогда
			СписокВыбора.Добавить("ФНС 5.02", "ФНС 5.02");
			СписокВыбора.Добавить("ФНС 5.01 (УПД:Счет-фактура)", "ФНС 5.01 (УПД:Счет-фактура)");
		КонецЕсли;
		СписокВыбора.Добавить("ФНС 5.01 (УПД:Счет-фактура и первичный документ)", "ФНС 5.01 (УПД:Счет-фактура и первичный документ)");
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		Если Не Объект.ИспользоватьУКД Тогда
			СписокВыбора.Добавить("ФНС 5.02", "ФНС 5.02");
			СписокВыбора.Добавить("ФНС 5.01 (УКД:Корректировочный счет-фактура)", "ФНС 5.01 (УКД:Корректировочный счет-фактура)");
		КонецЕсли;
		СписокВыбора.Добавить("ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)", "ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)");
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав") Тогда
		...
		СписокВыбора.Добавить("ФНС 5.01 (УПД:Первичный документ)", "ФНС 5.01 (УПД:Первичный документ)");
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		СписокВыбора.Добавить("ФНС 5.01", "ФНС 5.01");
		СписокВыбора.Добавить("ФНС 5.01 (УКД:Первичный документ)", "ФНС 5.01 (УКД:Первичный документ)");
	Иначе
		СписокВыбора.Добавить("CML 2.08", "CML 2.08");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'ФНС 5.01 (до 31.12.2016)'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'ФНС 5.01 (с 01.01.2016)'"));
...
Процедура ЗаполнитьПредставлениеРегламентаЭДО(ТекСтрока)
	...
	Если ТекСтрока.ИспользоватьЭП Тогда
	...
	ОтветнаяПодпись = "";
	Если ТекСтрока.ТребуетсяОтветнаяПодпись Тогда
...
Процедура ВывестиПредставленияТиповДокументов()
	...
	Если СоответствиеВидовИТипов <> Неопределено Тогда
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ЗаполнитьПоШаблонуНаСервере(Сред(Команда.Имя,10));
	ВывестиПредставленияТиповДокументов();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоШаблонуНаСервере(ИмяШаблона)

	СправочникОбъект = РеквизитФормыВЗначение("Объект",Тип("СправочникОбъект.СоглашенияОбИспользованииЭД"));
	СправочникОбъект.ВходящиеДокументы.Загрузить(ОбменСКонтрагентамиСлужебный.ТаблицаПредопределенногоПрофиля(ИмяШаблона));
