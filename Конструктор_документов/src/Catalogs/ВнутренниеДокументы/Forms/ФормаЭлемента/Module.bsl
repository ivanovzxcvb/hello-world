...
//кпд (
&НаКлиенте
Перем МассивРазделовИВариантов Экспорт;
//кпд )
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") И Не ВестиУчетСторон Тогда
		Если ПолучитьФункциональнуюОпцию("ВестиУчетПоОрганизациям",
			...
			Элементы.ГруппаОрганизация.Заголовок = НСтр("ru = 'Наша организация'");
			Элементы.ГруппаОрганизация.Подсказка = НСтр("ru = 'Организация'");
		Иначе
			Элементы.Организация.Видимость = Ложь;
			Элементы.ГруппаОрганизация.Заголовок = НСтр("ru = 'Наше предприятие'");
			Элементы.ГруппаОрганизация.Подсказка = НСтр("ru = 'Предприятие'");
		КонецЕсли;
	Иначе
		Элементы.Организация.Видимость = Ложь;
		Элементы.ГруппаОрганизация.Заголовок = НСтр("ru = 'Наше предприятие'");
...
Процедура ПриОткрытии(Отказ)
	...
	//кпд (
	МассивРазделовиВариантов = Новый Массив;
	//кпд )
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//кпд (
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И кпдКоличествоИтераций < ФайлыДобавленные.Количество() Тогда// И Не ПараметрыЗаписи.Свойство("ОкноКонструктораПоказано") Тогда 
		Конструктор = ПолучитьКонструктор(Объект.Шаблон, ФайлыДобавленные[кпдКоличествоИтераций].ШаблонОснованиеДляСоздания);
		кпдКоличествоИтераций = кпдКоличествоИтераций + 1;
		Если ЗначениеЗаполнено(Конструктор) Тогда 
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ПередЗаписьюПродолжениеПослеПоказаКонструктора",
				ЭтотОбъект,
				ПараметрыЗаписи);
			
			ОткрытьФорму("ОбщаяФорма.кпдКонструкторДокументов", Новый Структура("Конструктор, ШаблонОснование", Конструктор, ФайлыДобавленные[кпдКоличествоИтераций-1].ШаблонОснованиеДляСоздания), ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
//кпд (
&НаКлиенте
Процедура ПередЗаписьюПродолжениеПослеПоказаКонструктора(РазделыИВарианты, ПараметрыЗаписи) Экспорт 
	
	Если РазделыИВарианты = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	ПараметрыЗаписи.Вставить("ОкноКонструктораПоказано");
	ПараметрыЗаписи.Вставить("РазделыИВарианты", РазделыИВарианты);
	МассивРазделовиВариантов.Добавить(ПараметрыЗаписи.РазделыИВарианты); 
	Записать(ПараметрыЗаписи);
			...
			Если Источник <> Неопределено И ФайлТаблицы.ДобавленИзШаблона Тогда
				...
				Если ИспользоватьАвтозаполнениеФайлов Тогда
					...
					Иначе
						//TODO
						//кпд (
						НовыеДвоичныеДанныеФала = Неопределено;
						врРасширение = "docx"; 
						ИтоговыйФайл = ПолучитьИмяВременногоФайла("." + врРасширение);
						//ИмяКаталога = ФайловыеФункцииСлужебныйКлиент.РабочийКаталогПользователя();
						Если ПараметрыЗаписи.Свойство("РазделыИВарианты") Тогда
							Для Каждого Раздел Из ПараметрыЗаписи.РазделыИВарианты Цикл
								//TODO желательно сравнивать ссылки
								Если Строка(Раздел[0].ШаблонОснование) = Строка(Приемник) Тогда
									РазделыИВарианты = Раздел;
									Прервать;
								КонецЕсли;
							КонецЦикла;
							ФайлыДляСклейки = ПолучитьФайлыДляСклейки(РазделыИВарианты);
							//получаем файлы из рабочего каталога, создаем временный итоговый файл
							Если ФайлыДляСклейки <> Неопределено Тогда
								//врРасширение = ФайлДляАвтоЗаполнения.НастройкиЗаполнения[0].Значение.РасширениеШаблона;
								...
								Для Каждого Элемент Из ФайлыДляСклейки Цикл
									Если Не ЗначениеЗаполнено(Элемент.ФайлСсылка) Тогда
										Продолжить;
								КонецЕсли;
						//
						//		МассивЗамен = АвтозаполнениеШаблоновФайловСервер.ПолучитьМассивАвтозаполненийШаблона(
						//Элемент.ФайлСсылка, Элемент.ШаблонФайла, Объект.Ссылка);
						
						ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляОткрытия(Элемент.ФайлСсылка);
						ДанныеФайла.НаЧтение = Ложь;
						
						
						ПолныйПутьКФайлу = ДанныеФайла.ПолноеИмяФайлаВРабочемКаталоге;
						Если НЕ ЗначениеЗаполнено(ПолныйПутьКФайлу) Тогда
							//РаботаСФайламиКлиент.ПолучитьФайлВерсииВРабочийКаталог(Неопределено, ДанныеФайла, "");
							//ПолныйПутьКФайлу = ДанныеФайла.ПолноеИмяФайлаВРабочемКаталоге;
						КонецЕсли;
						ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ДанныеФайла.ПолноеИмяФайлаВРабочемКаталоге);
						...
						//	ДвоичныеДанныеЗаполненногоФайла.Записать(ПолныйПутьКФайлу);
						//	
						//КонецЕсли;
						
						Если Документ = Неопределено Тогда 
							Документ = MSWord.Application.Documents.Open(ПолныйПутьКФайлу);
							Документ.Activate();
							Документ.Select();
							Документ.ActiveWindow.Selection.Collapse(0);
						Иначе
							Документ.ActiveWindow.Selection.InsertFile(ПолныйПутьКФайлу,,,0);
							Документ.ActiveWindow.Selection.Collapse(0);
						КонецЕсли;
						
						//РаботаСФайламиКлиент.УдалитьФайлИзРабочегоКаталога(Неопределено, ДанныеФайла.Ссылка, Истина);
					КонецЦикла;
					
					Build = MSWord.Build;
					Если Найти(MSWord.Build, "12.") = 1 ИЛИ Найти(MSWord.Build, "14.") = 1 ИЛИ Найти(MSWord.Build, "16.") = 1 Тогда
						Если врРасширение = "docx" Тогда
							Документ.SaveAs(ИтоговыйФайл, 12);                        
						Иначе
							Документ.SaveAs(ИтоговыйФайл, 0);
						КонецЕсли;
					Иначе
						Если врРасширение = "docx" Тогда
							Документ.SaveAs(ИтоговыйФайл, 109);                        
						Иначе
							Документ.SaveAs(ИтоговыйФайл);
						КонецЕсли;
					КонецЕсли;
					
					Документ.Saved = Истина;
					Документ.Close();
					
					MSWord.Quit();
					MSWord = Неопределено;
					
					//НовыеДвоичныеДанныеФала = Новый ДвоичныеДанные(ИтоговыйФайл);
					//РезультатЗаполнения = Новый Структура;
					//РезультатЗаполнения.Вставить("Результат", Истина);
					//РезультатЗаполнения.Вставить("ДвоичныеДанныеЗаполненногоФайла", НовыеДвоичныеДанныеФала);
					РезультатЗаполнения = АвтозаполнениеШаблоновФайловКлиентСервер.ЗаполнитьПоляФайлаДаннымиВладельца(
							Ложь, Приемник, Истина,, ФайлыДляСклейки[0].ФайлСсылка.ВладелецФайла);
...
Процедура НачатьЭДОЗаново(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда  
		...
		ТекстВопроса = ТекстВопросаНезаписанныхДанных(НСтр("ru = 'Начать ЭДО заново'"));
...
Процедура ОтправитьПоЭДО(Команда)
	...
	Если КоличествоФайлов = 0 Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для отправки по ЭДО нужно добавть файл!'"));
			Возврат;
	КонецЕсли;
...
Процедура ПослеЗаписиКлиент(ПараметрыЗаписи)
	...
	Если ДанныеДляЗаполненияФайловНаКлиенте.Количество() > 0 Тогда
		...
		Для Каждого ФайлДляАвтоЗаполнения Из ДанныеДляЗаполненияФайловНаКлиенте Цикл
			...
			НовыеДвоичныеДанныеФала = Неопределено;
			Если ПараметрыЗаписи.Свойство("РазделыИВарианты") Тогда
				Для Каждого Раздел Из МассивРазделовИВариантов Цикл
					//TODO желательно сравнивать ссылки
					Если Строка(Раздел[0].ШаблонОснование) = Строка(ФайлДляАвтоЗаполнения.Файл) Тогда
						РазделыИВарианты = Раздел;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				...
				Если ФайлыДляСклейки <> Неопределено Тогда
					...
					Для Каждого Элемент Из ФайлыДляСклейки Цикл
						...
						//+ нужно добавить файл в рабочий каталог и потом удалить
						//тут нужна проверка на ДанныеФайла.ТипХраненияФайла потом доделать!!! 
						
						//+ нужна будет проверка расширения файла 
						...
						//Если Элемент <> ФайлыДляСклейки[ФайлыДляСклейки.Количество()-1] Тогда  
			...
			//кпд (
			Если НовыеДвоичныеДанныеФала <> Неопределено Тогда 
				ЗаполненныйФайл.ДвоичныеДанные = НовыеДвоичныеДанныеФала;
	...
	Если ПараметрыЗаписи.Свойство("ОтправитьПоЭДО") Тогда
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ЭДО. Состояние документа : %1'"),Строка(СостояниеВерсииДокументаПоЭДО));;
			Состояние(Текст, , , БиблиотекаКартинок.ЭмблемаСервиса1СЭДО);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьВидимостьПоЭДО()
	...
	Если Не НаправлениеИсходящий Тогда
		...
		Если ВидЭД = Перечисления.ВидыЭД.СчетФактура
			...
			Элементы.КомандаОтклонитьЭД.Заголовок = НСтр("ru = 'Запросить уточнение по электронному документу'");
...
Процедура ОбновитьЗаголовокПроцессаУтверждение()
	...
	Если ИспользоватьПодписание Тогда
		ЗаголовокПроцесса = НСтр("ru = 'На подписание'");
	Иначе
		ЗаголовокПроцесса = НСтр("ru = 'На утверждение'");
	КонецЕсли;
...
//кпд (
&НаСервереБезКонтекста
Функция ПолучитьКонструктор(Шаблон, ШаблонОснованиеДляСоздания)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ШаблоныВнутреннихДокументовПравилаАвтозаполнения.кпдКонструкторДокумента
	|ИЗ
	|	Справочник.ШаблоныВнутреннихДокументов.ПравилаАвтозаполнения КАК ШаблоныВнутреннихДокументовПравилаАвтозаполнения
	|ГДЕ
	|	ШаблоныВнутреннихДокументовПравилаАвтозаполнения.Ссылка = &Шаблон
	|	И НЕ ШаблоныВнутреннихДокументовПравилаАвтозаполнения.кпдКонструкторДокумента.ПометкаУдаления
	|	И ШаблоныВнутреннихДокументовПравилаАвтозаполнения.ШаблонФайла = &ШаблонОснованиеДляСоздания";
	Запрос.УстановитьПараметр("Шаблон", Шаблон);
	Запрос.УстановитьПараметр("ШаблонОснованиеДляСоздания", ШаблонОснованиеДляСоздания);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда 
		Возврат Выборка.кпдКонструкторДокумента;
	КонецЕсли;
	
	Возврат Неопределено;
	
КонецФункции // ПолучитьКонструктор()

&НаСервереБезКонтекста
Функция ПолучитьФайлыДляСклейки(РазделыИВарианты)
	
	УстановитьПривилегированныйРежим(Истина);
	
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("Раздел", Новый ОписаниеТипов("СправочникСсылка.кпдРазделыДокументов"), "Раздел");
	КвалификаторыСтроки = Новый КвалификаторыСтроки(100);
	ТаблицаЗначений.Колонки.Добавить("ВариантРаздела", Новый ОписаниеТипов("Строка",,КвалификаторыСтроки), "Вариант раздела");
	
	// добавим строку
	Для каждого Строка Из РазделыИВарианты Цикл
		Стр = ТаблицаЗначений.Добавить();
		Стр.Раздел = Справочники.кпдРазделыДокументов.НайтиПоНаименованию(Строка.Раздел, Истина);
		Стр.ВариантРаздела = Строка.ВариантРаздела;
	КонецЦикла;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	кпдРазделыДокументовВарианты.ФайлСсылка,
	|	кпдРазделыДокументовВарианты.ПравилоАвтозаполнения.ШаблонФайла КАК ШаблонФайла
	|ИЗ
	|	ВТДанные КАК ВТДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.кпдРазделыДокументов.Варианты КАК кпдРазделыДокументовВарианты
	|		ПО ВТДанные.Раздел = кпдРазделыДокументовВарианты.Ссылка
	|			И ВТДанные.ВариантРаздела = кпдРазделыДокументовВарианты.НазваниеВарианта
	|ГДЕ
	|	НЕ кпдРазделыДокументовВарианты.ФайлСсылка ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
	|	кпдРазделыДокументовВарианты.Ссылка.Порядок";
	...
	Пока Выборка.Следующий() Цикл
		...
		Структура_.Вставить("ФайлСсылка", Выборка.ФайлСсылка);
		Структура_.Вставить("ШаблонФайла", Выборка.ШаблонФайла);
		МассивВерсийФайлов.Добавить(Структура_);
	КонецЦикла;
	
	Возврат МассивВерсийФайлов;
	
КонецФункции // ПолучитьФайлыДляСклейки()
//кпд )