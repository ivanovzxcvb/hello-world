...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Запрос.Текст =
		...
		|	ПодчиненныеЭД.ТипЭлементаВерсииЭД КАК ТипЭДПодтверждения
		|ИЗ
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Результат.СледующийПоЗначениюПоля("ЭлектронныйДокумент") Цикл
			...
			СтрокаТЗОписи.КНД = ОбменСКонтрагентамиПовтИсп.КНДПоВидуЭД(Результат.ВидЭД);
			...
			Если ЗначениеЗаполнено(Результат.ПодтверждениеСсылка) Тогда
				...
				СтрокаТЗОписи.КНДПодтверждения = ОбменСКонтрагентамиПовтИсп.КНДПоВидуЭД(Результат.ТипЭДПодтверждения);
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Попытка
		...
		ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(Файл, "ВерсФорм", "1.02", Истина, ТекстОшибки);
