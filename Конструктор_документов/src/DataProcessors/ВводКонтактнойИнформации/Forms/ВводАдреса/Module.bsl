...
&НаКлиенте
Процедура АдренсыйКлассификаторУстарел(Команда)
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС +
			НСтр("ru = 'Для обновления адресного классфикатора необходимо обратиться к администратору.'");
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
				...
				Кнопки = Новый СписокЗначений();
				Кнопки.Добавить(КодВозвратаДиалога.ОК, "ОК");
				Кнопки.Добавить("ОбновитьКлассификатор", "Обновить классификатор");
				Оповещение = Новый ОписаниеОповещения("ПослеВопросОбОбновление", ЭтотОбъект);
				ПоказатьВопрос(Оповещение, СообщениеОПроверкеАдреса, Кнопки,, КодВозвратаДиалога.ОК);
			Иначе
				...
				|Для обновления адресного классфикатора необходимо обратиться к администратору.'");
				ПоказатьПредупреждение(, СообщениеОПроверкеАдреса);
			КонецЕсли;
		Иначе
