	...
	ИначеЕсли objectId.type <> "" Тогда 
		...
		Если Строка = Неопределено Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Попытка получить ссылку 1С:Документооборота для неизвестного типа XDTO: %1'"), objectId.type);
		КонецЕсли;
		...
		Иначе	
			Если Найти(ИмяТипа, "Перечисления.") > 0 Тогда 
				...
				Исключение
					ВызватьИсключение СтрШаблон(НСтр("ru = 'Нет значения ""%1"" среди значений типа ""%2""'"), objectId.id, ИмяТипа);
				КонецПопытки;
	...
	Для Каждого ОписаниеРеквизита Из ОписаниеДополнительныхРеквизитов Цикл
		...
		Для Каждого ТипРеквизита Из ОписаниеРеквизита.ТипРеквизита.Типы() Цикл
			...
			ИначеЕсли ТипРеквизита = Тип("СправочникСсылка.Организации") Тогда
				...
				ОбъектТипXDTO.presentation = НСтр("ru = 'Организация'");
