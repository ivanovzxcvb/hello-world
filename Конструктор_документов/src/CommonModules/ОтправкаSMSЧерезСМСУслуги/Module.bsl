...
Функция ОтправитьSMS(НомераПолучателей, Текст, ИмяОтправителя, Логин, Пароль) Экспорт
	...
	Для Каждого Элемент Из НомераПолучателей Цикл
		Получатель = ФорматироватьНомер(Элемент);
	...
	Если КодРезультата = "1" Тогда
		...
		Для Каждого Получатель Из Получатели Цикл
			ОтправленноеСообщение = Новый Структура("НомерПолучателя,ИдентификаторСообщения", Получатель,
				Получатель + "/" + ИдентификаторСообщения);
