...
Процедура ОпределитьЯвляетсяЛиКонтрагентИностранным(ДанныеКонтрагента, ЯвляетсяИностранным) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(КонтрагентСсылка)) И КонтрагентСсылка <> Справочники.Контрагенты.ПустаяСсылка() Тогда
		// Ссылка
		ЯвляетсяИностранным = (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентСсылка, "ЮрФизЛицо") = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент);
		
	Иначе
		// Объект
		ЯвляетсяИностранным = (КонтрагентСсылка.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент);
...
#Область ЗаполнениеАдресаEMailКонтрагента
	...
	АдресПоДаннымПрограммы    = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
		Контрагент, Справочники.ВидыКонтактнойИнформации.ЮридическийАдресКонтрагента);
	...
	ИнформацияОСвязях.ДанныеПрограммы.Вставить("Email", УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
		Контрагент, Справочники.ВидыКонтактнойИнформации.EmailКонтрагента));
...
Процедура ЗаполнитьИдентификаторКонфигурации(ИдентификаторКонфигурации) Экспорт
	...
	ИдентификаторКонфигурации = "DocMngCorp";
