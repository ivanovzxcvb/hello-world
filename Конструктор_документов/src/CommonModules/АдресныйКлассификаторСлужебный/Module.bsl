...
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Если СтрСравнить(Источник, "Сервис1С") = 0 Тогда
		...
		Если ЗначениеЗаполнено(Результат.КодыАдреса.КодКЛАДР) Тогда
			...
			ДлинаКодаКЛАДР = СтрДлина(КодКЛАДР);
			
			Если ДлинаКодаКЛАДР = 17 Тогда
				Результат.КодыКЛАДР.Улица = КодКЛАДР;
				КодКЛАДР = Лев(КодКЛАДР, 13);
			КонецЕсли;
			
			Если ДлинаКодаКЛАДР = 12 Тогда
				КодКЛАДР = "0" + КодКЛАДР;
			КонецЕсли;
				
			Если ДлинаКодаКЛАДР = 13 Тогда
				...
				Если ЗначениеЗаполнено(Адрес.СвРайМО) Тогда
					Результат.КодыКЛАДР.Регион = Лев(КодКЛАДР, 5) + "00000000";
				КонецЕсли;
