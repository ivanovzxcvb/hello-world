...
// Предназначена для обработчиков полей формы динамических списков.
//
// Параметры:
//  СписокИлиТекущиеДанные - ПолеФормы - поле формы динамического списка для проверки текущих данных.
//                         - Неопределено, ДанныеФормыСтруктура, Структура - текущие данные для проверки.
//
// Возвращаемое значение:
//  Булево.
//
Функция ЭтоЭлементДинамическогоСписка(СписокИлиТекущиеДанные) Экспорт
	
	Если ТипЗнч(СписокИлиТекущиеДанные) = Тип("ПолеФормы") Тогда
		ТекущиеДанные = СписокИлиТекущиеДанные.ТекущиеДанные;
	Иначе
		ТекущиеДанные = СписокИлиТекущиеДанные;
	КонецЕсли;
...
Процедура ИнтерактивнаяОбработкаПредупрежденийПриЗавершенииРаботы(Параметры, ПараметрыОткрытияФормы) Экспорт
	...
	ПараметрыОткрытияФормы.Свойство("РежимОткрытияОкна", РежимОткрытияОкна);
...
Процедура ОткрытьПрикладнуюФормуПредупреждения(Параметры, ОбработкаОтвета, ПредупреждениеПользователю, ИмяФормы, ПараметрыФормы)
	...
	Если ДействиеГиперссылка.Свойство("ПрикладнаяФормаПредупреждения", Форма) Тогда
		...
		ЗаданныеПараметрыФормы = Неопределено;
		Если ДействиеГиперссылка.Свойство("ПараметрыФормы", ЗаданныеПараметрыФормы) Тогда
			Если ТипЗнч(ЗаданныеПараметрыФормы) = Тип("Структура") И
				ЗаданныеПараметрыФормы.Свойство("РежимОткрытияОкна") Тогда 
				ПараметрыОткрытияФормы.Вставить("РежимОткрытияОкна", 
					ЗаданныеПараметрыФормы.РежимОткрытияОкна);
			КонецЕсли;
		КонецЕсли;	
