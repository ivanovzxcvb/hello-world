...
Функция ПолучитьПредставлениеЭД(СсылкаНаФайлЭД) Экспорт
	...
	Иначе
		...
		|ВидЭД, ДатаДокументаОтправителя, НомерДокументаОтправителя, Дата, ТипЭлементаВерсииЭД");
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	Иначе
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ТекстВидЭД = НСтр("ru = 'Произвольный электронный документ'");
		КонецЕсли;
	КонецЕсли;
			...
			ИначеЕсли НастройкиОбмена <> Неопределено Тогда
				...
				Если МассивСтатусов.Количество() > 0 Тогда
					...
					Иначе
						...
						ИначеЕсли СледующийСтатус = Перечисления.СтатусыЭД.ПодготовленоПодтверждение Тогда
...
Функция НастройкиОбменаЭД(СсылкаНаФайлЭД) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаФайлЭД) Тогда
		...
		РеквизитыФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаФайлЭД,
				"НаправлениеЭД, ПодписанЭП, ВладелецФайла, ТипЭлементаВерсииЭД");
		...
		Если ЗначениеЗаполнено(РеквизитыЭД) Тогда
			...
			НастройкиОбмена.Вставить("ИспользоватьПодпись",    РеквизитыФайла.ПодписанЭП);
...
Функция БилетНаСайтПоддержки(Аутентификация, ПоказыватьОшибки = Истина) Экспорт
	...
	Попытка
		...
		ПараметрыПодключения.ИмяПользователя =     Аутентификация.Логин;
		ПараметрыПодключения.Пароль =              Аутентификация.Пароль;
	...
	Если НЕ ПустаяСтрока(ДанныеФайла) Тогда
		...
		Если ИННКПП.Количество() >0 Тогда
			...
			Для каждого Строка Из ДанныеКонтрагентовДляПроверкиСервисом Цикл
				...
				КлючПоиска = Строка.ИНН + ?(ЗначениеЗаполнено(Строка.КПП), "/"+Строка.КПП, "");
...
Процедура СформироватьХМЛФайл(ТекЭлемент, МассивСтруктурВозврата, ПараметрыОбмена, ДопПараметры)
	...
	Для Каждого ОбъектДляВыгрузки Из МассивОбъектовДляВыгрузки Цикл
		...
		ИначеЕсли ТекЭлемент.Ключ = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель Тогда
			...
			Если НастройкиОбмена.ВерсияФормата = ВерсияФНС_2015 ИЛИ НастройкиОбмена.ВерсияФормата = ВерсияСхемыCML2 Тогда
		...
		ИначеЕсли ТекЭлемент.Ключ = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			...
			СтруктураВозврата = ОбменСКонтрагентамиДОВызовСервера.СформироватьЭДПоДокументуДО(ОбъектДляВыгрузки, НастройкиОбмена, МассивСтруктурВозврата);
...
Процедура УстановитьСсылкуДляВладельцаВРегистреСостояний(СсылкаНаОбъект, ЭлектронныйДокумент, ТипДокумента = "") Экспорт
...
Функция СоздатьЭлектронныйДокумент(СтруктураЭД = Неопределено) Экспорт 
	...
	Если СтруктураЭД.Свойство("ДополнительнаяИнформация") Тогда
		НовыйЭлектронныйДокумент.Комментарий = СтруктураЭД.ДополнительнаяИнформация;
	КонецЕсли;
...
// Процедура - Сохраняет текст причины отклонения в входящем или исходящем электронном документе.
//
// Параметры:
...
// Функция - Проверяет является ли переданный тип ЭД типом извещения о получении
//
// Параметры:
...
// Функция - Проверяет является ли переданный тип ЭД типом уведомления об уточнении.
//
// Параметры:
//  ТипЭлементаВерсииЭД	 - ПеречислениеСсылка.ТипыЭлементовВерсииЭД - тип ЭД для проверки.
// 
...
// Функция - Проверяет является ли переданный тип ЭД типом подтверждения получения.
//
// Параметры:
//  ТипЭлементаВерсииЭД	 - ПеречислениеСсылка.ТипыЭлементовВерсииЭД - тип ЭД для проверки.
// 
...
// Функция - проверяет возможность удаления электронного документа по его состоянию
//
// Параметры:
...
// Функция - ищет среди присоединенных файлов электронного документа отправленное уведомление об уточнении.
//
// Параметры:
...
// Функция - проверяет наличие подписи для второго титула электронного документа
//
// Параметры:
...
// Функция - получает из табличной части "Входящие электронные документы" настройки ЭДО способ обработки 
...
// Функция - Формирует список способов отражения в учете входящего электронного документа определенного вида.
//
// Параметры:
...
// Функция - требуется для получения имени документа для открытия форм по способу обработки входящего документа.
//
// Параметры:
...
// Функция - Формирует таблицу способов отражения входящих документов по имени профиля.
//
// Параметры:
...
Функция ОжидаемоеДействиеПоТекущемуСтатусу(Строка) Экспорт
...
// Процедура - Добавляет на форму страницы для отображения электронных документов и реквизиты для хранения табличных документов.
//
// Параметры:
