...
//           Формат "ОбщийМодуль.<ИмяОбщегоМодуля>.<ИмяПроцедуры>" используется когда процедура размещена в общем модуле.
...
//           Для клиентских обработчиков в имени процедуры рекомендуется использовать префикс "Подключаемый_<ИмяПроцедуры>",
//           чтобы было понятно что эта процедура используется неявно.
...
Функция ОписаниеКоманды(ИмяКомандыВФорме, АдресНастроек) Экспорт
	...
	ПолучатьФорму = ЗначениеЗаполнено(ОписаниеКоманды.ИмяФормы);
	ОписаниеКоманды.Вставить("Серверная", Не ПолучатьФорму);
	Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) И Не ПолучатьФорму Тогда
		ОписаниеКоманды.Серверная = Истина;
		...
		КонецЕсли;
		Если СтрЧислоВхождений(ОписаниеКоманды.Обработчик, ".") = 1 Тогда
			ОписаниеКоманды.Обработчик = "ОбщийМодуль." + ОписаниеКоманды.Обработчик;
		КонецЕсли;
		...
		Если Вид = "ОБЩИЙМОДУЛЬ" Тогда
			ОбъектМетаданныхОбщийМодуль = Метаданные.ОбщиеМодули.Найти(МассивПодстрок[1]);
			Если ОбъектМетаданныхОбщийМодуль = Неопределено Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Общий модуль ""%1"" не найден.'"), МассивПодстрок[1]);
			КонецЕсли;
