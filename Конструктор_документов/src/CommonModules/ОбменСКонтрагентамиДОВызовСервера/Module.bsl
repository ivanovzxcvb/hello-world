...
Функция СобытиеЖурналаРегистрации()
	...
	СобытиеЖурналаРегистрации = НСтр("ru = 'Обмен с контрагентами по ЭДО'",
...
Функция ПодготовитьСтруктуруДляДокументаДОПоЭД(ЭД)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение реквизитов документа на основе ЭД'"), УровеньЖурналаРегистрации.Ошибка,,,
...
Функция СоздатьДокументДО(РеквизитыДокумента)
	...
	Исключение
		Если Не ВнешняяТранзакция Тогда
			ОтменитьТранзакцию();
		КонецЕсли;
 		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание документа на основе ЭД'"), УровеньЖурналаРегистрации.Ошибка, 
...
Функция СоздатьДокументДОпоЭД(ДокументЭД) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание документа ДО на основе ЭД'"), УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
	...
	Если ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьОбменЭД") Тогда
		...
		ИначеЕсли ВыводитьСообщение Тогда
			Если РезультатЗапроса[1].Выгрузить()[0].Количество = 0 Тогда
				ПричинаНеГотовности = НСтр("ru = 'Документ не подписан с нашей стороны.'");
				КлючКДанным = "Объект.Стороны[0].Подписан";
			ИначеЕсли РезультатЗапроса[2].Выгрузить()[0].Количество = 0 Тогда
				ПричинаНеГотовности = НСтр("ru = 'Нет действующих соглашений по ЭДО.'");
			ИначеЕсли РезультатЗапроса[3].Выгрузить()[0].Количество = 0 Тогда
				ПричинаНеГотовности = НСтр("ru = 'Между сторонами нет действующего соглашения по ЭДО.'");
			ИначеЕсли РезультатЗапроса[4].Выгрузить()[0].Количество = 0 Тогда
				ПричинаНеГотовности = НСтр("ru = 'У документа нет ни одного файла.'");
				КлючКДанным = "ГруппаДанныеФайла";
			ИначеЕсли РезультатЗапроса[5].Выгрузить()[0].Количество = 0 Тогда
				ПричинаНеГотовности = НСтр("ru = 'Нет подходящего сертификата ЭП.'");
			Иначе 
				ПричинаНеГотовности = НСтр("ru = 'Ни у одного из файлов документа нет ЭП с сертификатом из соглашения по ЭДО.'");
...
Процедура СозданиеДокументовДОПоНеОбработаннымВхЭД();
	...
	Попытка
		...
		Для каждого ЭД Из МассивСсылокНаОбъект Цикл
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение документа ДО на основе ЭД и установка состояния'"), УровеньЖурналаРегистрации.Ошибка,,,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЦикла; 
	Исключение
...
Процедура ЗаполнитьДополнительныеДанныеПоДокументуОснования(ДокументОснование, ДополнительныеДанные) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыДокумента.ПорядокПродления) Тогда
		НоваяСтрока = ДополнительныеДанные.Добавить();
		НоваяСтрока.Имя = "ИндексПорядкаПродления";
		НоваяСтрока.Значение = Перечисления.ПорядокПродления.Индекс(РеквизитыДокумента.ПорядокПродления);
	КонецЕсли;
...
Функция ВозратитьСтруктуруДляПросмтораЭД(АдресХранилища) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Чтение файла xml'"), УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	ЧтениеXML.Закрыть();
...
Процедура НачатьЭДОЗаново(ДокументДО, Контрагент, НаправлениеЭД) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Начать ЭДО заново'"), УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
