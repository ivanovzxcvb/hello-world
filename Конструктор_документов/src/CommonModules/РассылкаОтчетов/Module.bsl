...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Оперативно"; // Малые объемы данных, не блокирует работу, технологический (не меняет пользовательские данные).
	Обработчик.ОбщиеДанные     = Ложь;
	Обработчик.Версия          = "2.3.3.30";
	Обработчик.Комментарий     = НСтр("ru = 'Отключает не используемые регламентные задания рассылок отчетов.'");
...
Процедура АктуализироватьСписокРегламентныхЗаданий() Экспорт
	Запрос = Новый Запрос;
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если РежимТестирования Тогда
		ОтменитьТранзакцию(); // Удалить все, что создали.
	КонецЕсли;
