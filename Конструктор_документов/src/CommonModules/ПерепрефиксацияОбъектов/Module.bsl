...
Функция НовыйКодОбъекта(Знач ФорматНовогоКода, Знач Код, Знач ЭтоДокумент, Объект)
	...
	Если КоличествоЛидирующихНулей < 0 Тогда
		...
		СтрокаСообщения = СтрШаблон(СтрокаСообщения, ?(ЭтоДокумент, НСтр("ru = 'номера'"), НСтр("ru = 'кода'")),
	...
	Иначе
		...
		ФорматнаяСтрока = СтрШаблон(ФорматнаяСтрока, Строка(ДлинаКодаСЛидирующимиНулями));
