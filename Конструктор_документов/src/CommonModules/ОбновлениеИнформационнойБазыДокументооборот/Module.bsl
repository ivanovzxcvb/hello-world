...
Процедура ПриДобавленииПодсистемы(Описание) Экспорт
	...
	Описание.Имя = "ДокументооборотКОРП";
	Описание.Версия = "2.1.10.2";
...
Функция ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка признака ""Вести учет по организациям"" для видов внутренних документов.'");
...
Процедура ПерейтиНаВерсию_2_1_8_8() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ошибка добавления поля ""Стороны"" в настройки доступности по состоянию.'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура СоздатьОрганизацию()
	...
	Иначе
		ОрганизацияОбъект.Наименование = НСтр("ru = 'Наша организация'; en = 'Our enterprise'");
	КонецЕсли;
