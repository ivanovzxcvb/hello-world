...
//  КолРаспакованныхЭД - Число - количество пакетов, которые удалось распаковать.
//
Процедура РаспаковатьПакетыЭДНаСервере(Знач МассивПакетовЭД, КолРаспакованныхЭД) Экспорт
	...
	// подписание и отправка служебных ЭД
...
//  МассивСлужебныхЭД	 - Массив - список служебных данных.
//  КолРаспакованныхЭД	 - Число - количество распакованных пакетов.
//
				...
				Если ЭлектронныйДокумент.ТипДанных = "ЭД" Тогда
					...
					Если ДобавленныйФайл = Неопределено Тогда
						...
						Если ВладелецЭД = Неопределено Тогда
							...
							Если ЭлектронныйДокумент.Свойство("ДополнительныеПараметры", ДополнительныеПараметры)
								...
								Если ДополнительныеПараметры.Свойство("Комментарий", Комментарий)
									...
									СтруктураВладельцаЭД.Вставить("ДополнительнаяИнформация", Комментарий);
								КонецЕсли;
							КонецЕсли;
							...
							Иначе
								...
								СтруктураВладельцаЭД.Вставить("Текст",                         ЭлектронныйДокумент.ТекстПроизвольногоЭД);
								...
								Если ЗначениеЗаполнено(ДокументОснование) Тогда
									СтруктураВладельцаЭД.Вставить("ДокументОснование", ДокументОснование);
								КонецЕсли;
								#КонецОбласти
						...
						Если ЗначениеЗаполнено(ДобавленныйФайл) Тогда
							...
							СтруктураЭД.Вставить("ДополнительныеПараметры", ЭлектронныйДокумент.ДополнительныеПараметры);
						...
						Если НЕ ОбменСКонтрагентамиСлужебный.ЭтоОтветныйТитул(СтруктураЭД.ТипЭлементаВерсииЭД) Тогда
							...
							Если Не ЭлектронныйДокумент.ДополнительныеПараметры.Свойство("НеТребуетсяИзвещение") Тогда
...
Процедура ПроверитьСертификатыПодписей(Контекст) Экспорт
	...
	Для Каждого СвойстваПакетаЭДО Из ДанныеПакетовЭДО Цикл
		...
		Для Каждого ДанныеПодписи Из ПодписиПакета Цикл
			...
			АдресПодписи = ДанныеПодписи.АдресЭП;
			ДвоичныеДанныеФайлаЭП = ПолучитьИзВременногоХранилища(АдресПодписи);
			Сертификаты = МенеджерКриптографии.ПолучитьСертификатыИзПодписи(ДвоичныеДанныеФайлаЭП);
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
			...
			Если ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Сформирован 
				...
				Если НЕ ЭтоСлужебныйДокумент Тогда 
					...
					Иначе
						...
						ИначеЕсли ЭтоИсходящийДокументСОтклонением(ТекСтрока) Тогда
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Если ДокументыКОтправке.Количество() > 0 Тогда
			...
			Если Не НемедленнаяОтправкаЭД  
				Или (ДопПараметры.Свойство("ФормироватьПакетыЭДПоПодписаннымЭД")
				И ДопПараметры.ФормироватьПакетыЭДПоПодписаннымЭД = Истина) Тогда
...
// Возвращает структуру, содержащую значения реквизитов Электронного документа прочитанные из информационной базы.
// 
...
//
Функция ПолучитьРеквизитыЭД(Ссылка) Экспорт
	
	РеквизитыЭД = Новый Массив;
	
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий")
		Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящий") Тогда 
		
		РеквизитыЭД.Добавить("ВидЭД");
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ПакетЭДПрисоединенныеФайлы") Тогда
		
		РеквизитыЭД.Добавить("ВладелецФайла");
		
	Иначе
		Возврат Новый Структура;
	КонецЕсли;
	
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, РеквизитыЭД)
...
Процедура УстановитьСостояниеЗавершенПоОтклонению(МассивФайловЭД)
	...
	|	Справочник.ЭДПрисоединенныеФайлы КАК ЭДПрисоединенныеФайлы
	|ГДЕ
	|	ЭДПрисоединенныеФайлы.НомерЭД В
	|			(ВЫБРАТЬ
	|				вт_НомераЭД.НомерЭД
	|			ИЗ
	...
	|	И НЕ ЭДПрисоединенныеФайлы.Ссылка В (&МассивФайловЭД)";
...
Процедура ЗаполнитьДокументИБПоЭД(СсылкаНаВладельца, СсылкаНаЭД, ДокЗаполнен =Ложь, ТипДокумента = "")
	...
	Если СтруктураЭД.Загружен Тогда
		...
		Попытка
			...
			ДокументыУчета = ОбменСКонтрагентамиВнутренний.СохранитьДанныеОбъекта(СтруктураПараметров, ТипДокумента);
			
			Если ЗначениеЗаполнено(ДокументыУчета) Тогда
				Для каждого Строка Из ДокументыУчета Цикл
					ОбменСКонтрагентамиСлужебный.УстановитьСсылкуДляВладельцаВРегистреСостояний(Строка, СсылкаНаЭД.ВладелецФайла, ТипДокумента);	
				КонецЦикла;
			КонецЕсли;
...
Функция ПолучитьИмяФормыЭД(СсылкаНаЭД) Экспорт
	...
	Если ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
		ИмяФормы = "Документ." + ИмяДокумента + ".Форма.ФормаПроизвольногоДокумента";
		СсылкаНаЭД = ЭлектронныйДокумент;
	Иначе
...
Функция ЭтоИсходящийДокументСОтклонением(СвойстваЭД)
	
	Результат = Ложь;
	
	МассивРТУ = Новый Массив;
	МассивРТУ.Добавить(Перечисления.ВидыЭД.ОтветНаЗаказ);
	МассивРТУ.Добавить(Перечисления.ВидыЭД.ЗаказТовара);
	МассивРТУ.Добавить(Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара);
	МассивРТУ.Добавить(Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара);
	МассивРТУ.Добавить(Перечисления.ВидыЭД.СчетНаОплату);
	МассивРТУ.Добавить(Перечисления.ВидыЭД.ПрайсЛист);
