...
Функция ПолучитьЗначениеРеквизитаПредмета(СсылкаНаПредмет, ИмяРеквизита) Экспорт
	...
	Если ТипЗнч(СсылкаНаПредмет) = Тип("СправочникСсылка.ЗаписиРабочегоКалендаря")
		И ИмяРеквизита = "ДатаНачала" Тогда
		Возврат ПолучитьЗначениеДатыНачалаЗаписиКалендаря(СсылкаНаПредмет);
	КонецЕсли;
...
Процедура ПроверитьИзмененияДатВПредметеПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНапоминанияПользователя") Тогда
		...
		ПроверитьУстановкуПометкиУдаления(Источник.Ссылка);
	КонецЕсли;
...
Функция ПодключитьНапоминаниеДоВремениПредмета(Текст, Интервал, Предмет, ИмяРеквизита,
	...
	Напоминание = СоздатьНапоминание(ПараметрыНапоминания, ВремяСобытия);
	ПодключитьНапоминание(Напоминание);
...
Функция СоздатьНапоминание(ПараметрыНапоминания, ВремяСобытия = Неопределено)
	...
	Если Напоминание.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета Тогда
		...
		Иначе
			Напоминание.ВремяСобытия =
				ПолучитьЗначениеРеквизитаПредмета(Напоминание.Источник, Напоминание.ИмяРеквизитаИсточника);
		КонецЕсли;
...
Процедура ПроверитьУстановкуПометкиУдаления(Предмет) Экспорт
	...
	ПометкаУдаления = ПолучитьЗначениеРеквизитаПредмета(Предмет, "ПометкаУдаления");
