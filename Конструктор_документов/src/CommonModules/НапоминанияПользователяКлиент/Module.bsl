...
Функция НайтиЗаписьВКэшеОповещений(КэшОповещений, ПараметрыОповещения)
	Для Каждого Запись Из КэшОповещений Цикл
		Если Запись.Источник = ПараметрыОповещения.Источник Тогда
			Возврат Запись;
		КонецЕсли;
	КонецЦикла;
...
// Проверяет открыта ли форма оповещения
Функция ОткрытаФормаОповещения() Экспорт
	...
	КонецЕсли;
	ФормаОповещения = ПараметрыПриложения[ИмяПараметра];
	Возврат ФормаОповещения.Открыта();
	
КонецФункции

// Открывает форму оповещения
Процедура ОткрытьФормуОповещенияДокументооборота(Напоминание) Экспорт
	
	ПараметрыФормы = Новый Структура("Напоминание", Напоминание);
	ФормаОповещения = ПолучитьФорму("РегистрСведений.НапоминанияПользователя.Форма.ФормаОповещенияДокументооборот",
		ПараметрыФормы, , Напоминание.Источник.УникальныйИдентификатор(), ВариантОткрытияОкна.ОтдельноеОкно);
