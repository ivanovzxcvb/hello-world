...
Процедура ЗарегистрироватьТипыИСвязи(Информация, УзелКД, ЭлементКД, НастройкаВарианта)
	...
	ТекстЗапросаСсылок = "";
	ШаблонЗапросаСсылок = "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ Ссылка ИЗ &ПолноеИмяТаблицы";
	ШаблонОбъединенияЗапросов = Символы.ПС + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС;
	...
	Для Каждого Тип Из ИнформацияОТипах.ОбъектныеТипы Цикл
		...
		ДляВыбораЭтогоТипаИспользуетсяБыстрыйВыбор = НастройкаВарианта.БыстрыйВыбор;
		Вид = ВРег(СтрЛев(ПолноеИмя, "."));
		Если Вид = "ПЕРЕЧИСЛЕНИЕ" Тогда
			ДляВыбораЭтогоТипаИспользуетсяБыстрыйВыбор = Истина;
		ИначеЕсли Вид = "СПРАВОЧНИК"
			...
			КонецЕсли;
		КонецЕсли;
		Если ДляВыбораЭтогоТипаИспользуетсяБыстрыйВыбор Тогда
			ИнформацияОТипах.КоличествоСБыстрымВыбором = ИнформацияОТипах.КоличествоСБыстрымВыбором + 1;
			ТекстЗапросаЭтогоТипа = СтрЗаменить(ШаблонЗапросаСсылок, "&ПолноеИмяТаблицы", ПолноеИмя);
			Если ТекстЗапросаСсылок = "" Тогда
				ТекстЗапросаСсылок = ТекстЗапросаЭтогоТипа;
			ИначеЕсли СтрНайти(ТекстЗапросаСсылок, ТекстЗапросаЭтогоТипа) = 0 Тогда
				ТекстЗапросаСсылок = ТекстЗапросаСсылок + ШаблонОбъединенияЗапросов + ТекстЗапросаЭтогоТипа;
			КонецЕсли;
		КонецЕсли;
	...
	Если ИнформацияОТипах.КоличествоСБыстрымВыбором = ИнформацияОТипах.КоличествоТипов Тогда
	...
	Иначе
		Если НастройкаВарианта.БыстрыйВыбор Тогда
			НастройкаВарианта.ЗапросЗначенийВыбора.Текст = ТекстЗапросаСсылок;
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее) Экспорт
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СвязьСКомпоновщиком" Тогда
			...
			ИначеЕсли Не СвойстваНастройки.БыстрыйВыбор Тогда
