...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если ПользовательПоИдентификаторуСуществует(ТекущийПользовательИБ.УникальныйИдентификатор) Тогда
...
//
// Параметры:
//  ГруппаПриемник     - СправочникСсылка.РабочиеГруппы - группа, в которую переносится
//                       пользователь.
//  ПользовательСсылка - СправочникСсылка.Пользователь - пользователь, которого необходимо
//                       добавить в группу.
...
//
// Параметры:
//  ГруппаПриемник     - СправочникСсылка.РабочиеГруппы - группа, из которой удаляется
//                       пользователь.
//  ПользовательСсылка - СправочникСсылка.Пользователь - пользователь, которого необходимо
//                       добавить в группу.
//  ТипПользователя    - Строка - ВнешнийПользователь или Пользователь.
...
Процедура ОбновитьСоставыИерархическихГруппПользователей(ГруппаПользователей,
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА РабочиеГруппыСостав.Ссылка.ПометкаУдаления
	|				ТОГДА ЛОЖЬ
	|			КОГДА РабочиеГруппыСостав.Пользователь.ПометкаУдаления
	|				ТОГДА ЛОЖЬ
	|			КОГДА РабочиеГруппыСостав.Пользователь.Недействителен
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ) КАК Используется
	...
	|		ПО (СсылкиВИерархииРодителей.Ссылка = РабочиеГруппыСостав.Ссылка)
	...
	|ГДЕ
	|	РабочиеГруппы.Ссылка = &ГруппаПользователей
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Иначе
		...
		ОтборПользователяВСправочнике = "РабочиеГруппыСостав.Пользователь В (&Пользователь)";
	КонецЕсли;
	...
	Если ОбновитьГруппыВнешнихПользователей Тогда
		...
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст,
			"РабочиеГруппыСостав.Пользователь",
			"РабочиеГруппыСостав.ВнешнийПользователь");
	КонецЕсли;
