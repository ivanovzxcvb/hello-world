...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	ИначеЕсли Статус = "Ошибка" Тогда
		...
		Если ВозвращатьРезультатВОбработкуВыбора() Тогда
			...
			ПараметрыПредупреждения.Вставить("Текст",     Задание.КраткоеПредставлениеОшибки);
			ПараметрыПредупреждения.Вставить("Подробно",  Задание.ПодробноеПредставлениеОшибки);
			ПараметрыПредупреждения.Вставить("Заголовок", НСтр("ru = 'Не удалось выполнить запрошенное действие'"));
			СтандартныеПодсистемыКлиент.ВывестиПредупреждение(ВладелецФормы, ПараметрыПредупреждения);
		КонецЕсли;
		Возврат;
